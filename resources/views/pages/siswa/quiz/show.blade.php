        ...
        <div class="col-lg-12">
            <div class="card p-0 radius-12 overflow-hidden">
                <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-3">
                    ...
                    <a href="{{ route('siswa.quiz.index') }}" class="btn btn-secondary">
                        ...
                        <div class="d-flex align-items-center gap-16 justify-content-between flex-wrap mb-24">
                            <div class="d-flex align-items-center gap-8">
                                ...
                                <div class="d-flex flex-column">
                                    <h6 class="text-lg mb-0">{{ $quiz->creator->name ?? '-' }}</h6>
                                    <span class="text-sm text-neutral-500">
                                        {{ \Carbon\Carbon::parse($quiz->created_at)->diffForHumans() }}</span>
                                </div>
                            </div>
                            <div class="d-flex align-items-center gap-md-3 gap-2 flex-wrap">
                                <div class="d-flex align-items-center gap-8 text-neutral-500 text-md fw-medium">
                                    ...
                                    {{ \Carbon\Carbon::parse($quiz->created_at)->translatedFormat('l, d F Y H:i') }}
                                </div>
                            </div>
                        </div>
                        <div class="col-12">
                            ...
                            <p>
                                <a href="{{ $quiz->link }}" target="_blank" class="btn btn-outline-primary btn-sm">
                                    <i class="fas fa-external-link-alt"></i> Buka Link Quiz
                                </a>
                            </p>
                        </div>
                        <h4 class="mb-16">{{ $quiz->title }}</h4>
                    </div>
                </div>
            </div>
        </div>
